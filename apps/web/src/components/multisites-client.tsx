...
export function MultisitesClient() {
  const { scenario, activeSite, decisions, decide, selectActiveSite } = useDemo();
  ...
  const suspendedSites = scenario.sites.filter((site) => site.expectedCovers === null).length;
  ...
  return <>
    <PageHeader eyebrow="Établissements · même monde fictif" title="Où agir dans le groupe" description={`${scenario.name} · trois établissements comparés au même instant ${scenario.asOf}. Vue active : ${activeSite.name}.`} site="Groupe Bordeaux" />

    <section className="group-summary" aria-label="Synthèse du groupe">
      <article><span>Dîner prévu</span><strong>{totalCovers}</strong><small>{suspendedSites ? `couverts connus · ${suspendedSites} site suspendu` : "couverts sur 3 sites"}</small></article>
    ...
    <section className="group-board" aria-labelledby="group-board-title">
      ...
      const tone = gap === null ? "warning" : gap < 0 ? "danger" : gap > 0 ? "warning" : "success";
          ...
          return <article className={`group-site-row ${tone} ${activeSite.id === site.id ? "active" : ""}`} role="row" key={site.id}>
            ...
            <div><span>Lecture</span><strong>{gap !== null && gap < 0 ? "Renfort à couvrir" : gap !== null && gap > 0 ? "Peut aider" : site.stockRisk === "Élevé" ? "Stock à sécuriser" : "Plan maintenu"}</strong>{activeSite.id === site.id ? <small className="active-site-label">Vue active</small> : <button className="site-focus-button" type="button" onClick={() => selectActiveSite(site.id)}>Mettre en vue active</button>}</div>
          </article>;
        })}
      </div>
    </section>
