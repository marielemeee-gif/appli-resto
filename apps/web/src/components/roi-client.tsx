...
export function RoiClient() {
  ...
  return <>
    ...
    <section className="table-panel" aria-labelledby="table-title"><h2 id="table-title">Historique · {activeSite.name}</h2>{rows.length === 0 ? <p>Aucune décision enregistrée pour {activeSite.name} dans cet instantané. Une consigne terrain peut être ajoutée depuis Décisions.</p> : <div className="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Décision</th><th>Responsable</th><th>Statut</th><th>Gain estimé</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td data-label="Date">{new Date(item.decidedAt).toLocaleString("fr-FR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</td><td data-label="Type">{typeLabels[item.recommendationType] ?? item.recommendationType}</td><td data-label="Décision">{item.title}{item.deadline ? <small className="table-deadline">avant {item.deadline}</small> : null}{item.note ? <small className="table-note">Motif : {item.note}</small> : null}</td><td data-label="Responsable">{item.owner ?? item.site}</td><td data-label="Statut">{labels[item.status]}</td><td data-label="Gain estimé">{item.estimatedGain} €</td></tr>)}</tbody></table></div>}</section>
  </>;
}
